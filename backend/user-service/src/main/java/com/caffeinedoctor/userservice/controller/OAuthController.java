...
//            "사용자 정보를 이용하여 서비스에 회원가입합니다.")
    public ResponseEntity<UserResponseDto> kakaoCallback(@RequestParam("code") String code) {
        // Kakao 토큰 요청
        String accessToken = kakaoOAuthService.requestKakaoToken(code);
        // Kakao 사용자 정보 요청
        UserResponseDto userInfo = kakaoOAuthService.requestKakaoUserInfo(accessToken);
